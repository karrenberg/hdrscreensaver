...
- Allow hotkey `DEL` to delete the currently-showing file (with confirmation dialog)
- Use the app icon in the window bar.
- Remove "Settings saved" confirmation dialog.
- Add option to installer to make HDRScreenSaver the default app to open supported image filetypes.
- Get rid of the launcher .scr now that no DLLs need to be copied anymore?
- Launching multiple instances (image viewer mode) makes the mouse sluggish, possibly other issues. 
...
- Make WebView2 installation optional?
